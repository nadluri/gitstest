...
namespace empRestAPI.Services
{
    public class EmployeeService : IEmployee
    {
        ...
        public async Task<string> AddEmployee(Employee employee)
        {
            try
            {
                if (string.IsNullOrEmpty(employee.EmployeeName) && employee.DepartmentID == 0)
                    return $"Employee name should not be null && Department ID should not be zero";
                if (string.IsNullOrEmpty(employee.EmployeeName))
                    return $"Employee name should not be null";
                if (employee.DepartmentID == 0)
                    return $"Dpartment ID should not be zero";
        ...
        {
            if (EmployeeID== 0)
                return $"Employee ID should not be zero";
